            ...
            <div class="rounded-3xl bg-white p-5 shadow-soft">
                ...
                <div class="mt-3 grid grid-cols-3 gap-3 rounded-2xl bg-slate-50 p-3">
                    ...
                    <div>
                        ...
                        <strong class="text-sm text-warning">{{ Currency::formatLkr($customer->pending_amount ?? 0) }}</strong>
                    </div>
                    <div class="text-right">
                        <span class="block text-xs font-semibold text-slate-400">Passed</span>
                        <strong class="text-sm text-success">{{ Currency::formatLkr($customer->passed_amount ?? 0) }}</strong>
                    </div>
                </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($customers as $customer)
                    <tr>
                        ...
                        <td class="font-bold text-warning">{{ Currency::formatLkr($customer->pending_amount ?? 0) }}</td>
                        <td class="font-bold text-success">{{ Currency::formatLkr($customer->passed_amount ?? 0) }}</td>
                        <td>
                            <span class="rounded-full px-3 py-1 text-xs font-bold {{ $customer->status === 'active' ? 'bg-green-100 text-success' : 'bg-red-100 text-danger' }}">
                                {{ ucfirst($customer->status) }}
                            </span>
